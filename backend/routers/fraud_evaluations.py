...
from backend.services.fraud_evaluation_service import (
    get_all_fraud_evaluations,
    get_fraud_evaluation,
)
...
def list_fraud_evaluations(
    ...
    decision: Literal["APPROVE", "REVIEW", "REJECT"] | None = Query(
        None,
        description="Filter evaluations by decision",
    ),
    min_score: int | None = Query(
        None,
        ge=0,
        description=(
            "Filter evaluations with total score greater than "
    ...
    max_score: int | None = Query(
        ...
        ge=0,
        description=(
            "Filter evaluations with total score less than "
            "or equal to this value"
        ),
    ),
    limit: int = Query(
...
def fraud_evaluation_by_id(
    ...
    evaluation = get_fraud_evaluation(
        db=db,
        id=id,
    )
